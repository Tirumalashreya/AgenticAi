...
from agents.google_adk.agent import MathSolverAgent
...
def main(host, port):
    ...
    skill = AgentSkill(
        id="math_solver",                                 # Unique skill ID
        name="Maths solving tool",                          # Human-friendly name
        description="Solves mathematical expressions and returns the result",    # What the skill does
        tags=["math", "calculator", "expression", "algebra", "arithmetic"],                               # Optional tags for searching
        examples=["What is 5 + 3?", "Solve 5(x - 4) + 2x", "Evaluate 2^3 * (4 + 1)"]
    )

    # Create an agent card describing this agent’s identity and metadata
    agent_card = AgentCard(
        name="MathSolverAgent",                               # Name of the agent
        description="This agent Solves mathematical expressions and returns the result.",  # Description
        url=f"http://{host}:{port}/",                       # The public URL where this agent lives
        version="1.0.0",                                    # Version number
        defaultInputModes=MathSolverAgent.SUPPORTED_CONTENT_TYPES,  # Input types this agent supports
        defaultOutputModes=MathSolverAgent.SUPPORTED_CONTENT_TYPES, # Output types it produces
    ...
    server = A2AServer(
        ...
        task_manager=AgentTaskManager(agent=MathSolverAgent())
    )
